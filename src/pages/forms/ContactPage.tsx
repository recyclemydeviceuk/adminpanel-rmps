...
const STATUS_STYLE: Record<ContactSubmission['status'], { pill: string; dot: string; label: string }> = {
  new:     { pill: 'bg-blue-50 text-blue-700 border-blue-200',     dot: 'bg-blue-500',     label: 'New'     },
  read:    { pill: 'bg-amber-50 text-amber-700 border-amber-200',   dot: 'bg-amber-500',    label: 'Read'    },
  replied: { pill: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500', label: 'Replied' },
};
...
export default function ContactPage() {
  ...
  const load = () => {
    ...
    getContactSubmissions()
      ...
      .catch((err) => {
        setSubs([]);
        setFetchError(err?.response?.data?.message || 'Failed to load submissions. Check your connection and try again.');
      })
  ...
  const newCount     = subs.filter((r: any) => r.status === 'new').length;
  const readCount    = subs.filter((r: any) => r.status === 'read').length;
  const repliedCount = subs.filter((r: any) => r.status === 'replied').length;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0f1117] via-[#1a1d2e] to-[#0f1117] px-7 py-6 shadow-xl">
        <div className="absolute -top-10 -right-10 h-44 w-44 rounded-full bg-blue-500/20 blur-3xl" />
        <div className="absolute -bottom-8 left-16 h-32 w-32 rounded-full bg-indigo-600/15 blur-2xl" />
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-[22px] font-black text-white tracking-tight">Contact Submissions</h1>
              <p className="text-[13px] text-white/50 mt-0.5">All messages submitted via the Contact Us form</p>
            </div>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 px-4 py-2 shadow-md shadow-blue-200/30">
              <span className="text-[12px] font-semibold text-white/80">New</span>
              <span className="text-[18px] font-black text-white leading-none">{newCount}</span>
            </div>
            <div className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 px-4 py-2 shadow-md shadow-amber-200/30">
              <span className="text-[12px] font-semibold text-white/80">Read</span>
              <span className="text-[18px] font-black text-white leading-none">{readCount}</span>
            </div>
            <div className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-green-500 px-4 py-2 shadow-md shadow-green-200/30">
              <span className="text-[12px] font-semibold text-white/80">Replied</span>
              <span className="text-[18px] font-black text-white leading-none">{repliedCount}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[#e8eaed] bg-white pl-9 pr-3 py-2.5 text-[13px] placeholder:text-[#9aa0a6] focus:outline-none focus:ring-2 focus:ring-blue-400"
          />
        </div>
        {search && (
        ...
        <div className="flex items-center gap-1 rounded-xl border border-[#e8eaed] bg-white p-1">
          {(['all', 'new', 'read', 'replied'] as const).map(s => (
            <button key={s} onClick={() => { setStatus(s); setPage(1); }}
              className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold transition-all capitalize ${statusFilter === s ? 'bg-blue-600 text-white shadow-sm' : 'text-[#5f6368] hover:bg-gray-50'}`}>
      ...
      <div className="rounded-2xl border border-[#e8eaed] bg-white overflow-hidden shadow-sm">
        ...
        {loading ? (
          <div className="flex flex-col items-center gap-3 py-20">
            ...
            <p className="text-[13px] text-[#9aa0a6]">Loading submissions…</p>
          </div>
        ) : paginated.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-20">
            ...
            <p className="text-[14px] font-semibold text-[#5f6368]">{fetchError ? 'Failed to load submissions' : 'No submissions found'}</p>
          </div>
        ) : (
          <div className="divide-y divide-[#f8fafc]">
            {paginated.map(row => {
              const st = STATUS_STYLE[row.status as ContactSubmission['status']];
                ...
                <div key={row.id}>
                  ...
                  <div className="grid grid-cols-[2fr_1.5fr_1fr_1fr_32px] gap-3 items-center px-5 py-3.5 hover:bg-[#fafbfc] transition-colors">
                    ...
                    <div className="flex items-center gap-1.5 min-w-0">
                      <Phone size={11} className="text-[#9aa0a6] flex-shrink-0" />
                      <span className="text-[12px] text-[#5f6368] font-mono truncate">{row.phone}</span>
                    </div>
                    {/* Status */}
                    ...
                    <div className="px-5 pb-4 pt-1 bg-[#fafbfc] border-t border-[#f1f3f4]">
                      <div className="rounded-xl border border-[#e8eaed] bg-white p-4">
                        <div className="flex items-center gap-2 mb-2.5">
                          ...
                          <span className="text-[11px] font-bold uppercase tracking-wide text-[#9aa0a6]">Message</span>
                        </div>
                        <p className="text-[13px] text-[#202124] leading-relaxed">{row.message}</p>
                      </div>
                    </div>
                  )}
                </div>
